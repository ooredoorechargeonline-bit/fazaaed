...
async function api(req, res, url) {
  ...
  if (req.method === "POST" && url === "/api/orders") {
    ...
    if (i >= 0) {
      const prev = orders[i];
      orders[i] = { ...prev, ...o, ts: prev.ts, status: prev.status === "confirmed" ? "confirmed" : o.status, decision: prev.decision, updated: Date.now() };
    } else {
  ...
  if (req.method === "GET" && url.startsWith("/api/status/")) {
    ...
    return send(res, 200, { ok: true, status: o.status, decision: o.decision || "" });
  }
  // نبضة زائر
  ...
  if (url.startsWith("/api/admin/")) {
    ...
    // الأدمن يقبل أو يرفض طلباً في انتظار الموافقة
    if (req.method === "POST" && /^\/api\/admin\/decide\//.test(url)) {
      ...
      if (!o) return send(res, 404, { ok: false });
      o.decision = decision;
      o.status = decision === "accept" ? "confirmed" : "rejected";
      o.updated = Date.now();
      saveOrders();
